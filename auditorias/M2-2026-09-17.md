...
1. [SEM PROVA] R9 — a spec (`specs/M2-inscricoes.md:57`) exige que `ocupadas` = inscrições `confirmada` + `convocada`, `vagasRestantes` = `vagas` − `ocupadas` (nunca negativa) e `emEspera` = inscrições `em_espera`, mas nenhum teste executa cenário consultando a atividade para verificar diretamente esses contadores derivados. Cenário que expõe: criar uma atividade com vagas limitadas, efetuar inscrições com estados variados (`confirmada`, `convocada`, `em_espera`) e realizar uma requisição GET na atividade para validar que os campos `ocupadas`, `vagasRestantes` e `emEspera` correspondem exatamente aos valores calculados em `api/verificacoes/inscricoes.spec.js`.

## Veredito

Pode ser aceito após a adição de teste específico para comprovar o cálculo dos contadores derivados da atividade (R9).
